...
void handle_xor64(CPU& cpu, const std::vector<uint8_t>& program, bool& running) {
    ...
    if (pc + 3 >= program.size()) {
        Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION,
            fmt::format("[PC=0x{:04X}] XOR64: Not enough bytes for instruction", pc), Logging::DebugLevel::CRITICAL);
        running = false;
        return;
    }
    ...
    Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION,
        fmt::format("[PC=0x{:04X}] [XOR64] R{} = R{} ^ R{}",
    ...
    if (dest_reg >= cpu.get_registers_64().size() ||
        ...
        Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION,
            fmt::format("[PC=0x{:04X}] XOR64: Register index out of bounds", pc), Logging::DebugLevel::CRITICAL);
        running = false;
        return;
    }
    ...
    Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION,
        fmt::format("[PC=0x{:04X}] [XOR64] Values: {} ^ {}",
    ...
    Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION,
        fmt::format("[PC=0x{:04X}] [XOR64] Result: R{} = {} (Flags: C={} O={} S={} Z={})",
        pc, dest_reg, result,
