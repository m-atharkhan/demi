...
void handle_FIST(CPU& cpu, const std::vector<uint8_t>& program, bool& running) {
    ...
    Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION, fmt::format("[PC=0x{:04X}] [FIST] operand_type=0x{:02X}", cpu.get_pc() - 2, operand_type), Logging::DebugLevel::DETAIL);
    ...
    switch (operand_type) {
        case 0x01: { // Memory address - store as 32-bit integer
            ...
            Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION, fmt::format("[FIST] Stored ST(0)={} as int32 {} to addr 0x{:04X}", float_value, int_value, addr), Logging::DebugLevel::DETAIL);
            break;
        }
        ...
        case 0x02: { // Memory address - store as 16-bit integer
            ...
            Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION, fmt::format("[FIST] Stored ST(0)={} as int16 {} to addr 0x{:04X}", float_value, int_value, addr), Logging::DebugLevel::DETAIL);
            break;
        }
        ...
        case 0x03: { // Memory address - store as 64-bit integer
            ...
            Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION, fmt::format("[FIST] Stored ST(0)={} as int64 {} to addr 0x{:04X}", float_value, int_value, addr), Logging::DebugLevel::DETAIL);
            break;
        }
        
        default:
            Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION, fmt::format("[FIST] Invalid operand type 0x{:02X}", operand_type), Logging::DebugLevel::CRITICAL);
            running = false;
            break;
    }
