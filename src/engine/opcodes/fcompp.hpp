...
//   ZF=0, CF=1: ST(0) < ST(1)
//   ZF=0, CF=0: ST(0) > ST(1)
inline void handle_FCOMPP(CPU& cpu, [[maybe_unused]] const std::vector<uint8_t>& program, [[maybe_unused]] bool& running) {
    ...
    Logging::DebugHandler::instance().report(Logging::DebugCategory::CPU_EXECUTION,
        fmt::format("[PC={:#06x}] [FCOMPP] compare {} vs {} -> ZF={} CF={}", 
        cpu.get_pc(), st0, st1,
        (flags & FLAG_ZERO) ? 1 : 0,
        (flags & FLAG_CARRY) ? 1 : 0), Logging::DebugLevel::DETAIL);
