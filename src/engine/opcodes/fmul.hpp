...
void handle_FMUL(CPU& cpu, const std::vector<uint8_t>& program, bool& running) {
    ...
    // Format: FMUL [source] - multiplies ST(0) by source
    // Operand types:
    ...
    switch (operand_type) {
        ...
        case 0x01: {
            // FMUL from memory (64-bit double)
            if (cpu.get_pc() + addr_size > program.size()) {
                running = false;
                return;
            }
