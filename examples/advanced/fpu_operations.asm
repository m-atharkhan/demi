; FPU Operations Test Program
; Tests basic floating-point operations in DemiEngine

; Test data: load double precision values
section .data
    ; Load immediate values into registers for FPU testing
    
section .text
start:
    ; Test 1: Load floating-point values
    LOAD_IMM64 EAX, 0x4059000000000000  ; Load 100.0 (IEEE 754 double)
    LOAD_IMM64 EBX, 0x4024000000000000  ; Load 10.0 (IEEE 754 double)
    
    ; Test 2: FPU Load operations
    FLD EAX          ; Load 100.0 to FPU stack (ST0)
    FLD EBX          ; Load 10.0 to FPU stack (ST0), 100.0 -> ST1
    
    ; Test 3: FPU Arithmetic
    FADD            ; ST0 = ST0 + ST1 = 10.0 + 100.0 = 110.0
    FST ECX          ; Store result to ECX
    
    FLD EAX          ; Load 100.0 again
    FLD EBX          ; Load 10.0 again
    FSUB            ; ST0 = ST0 - ST1 = 10.0 - 100.0 = -90.0
    FST EDX          ; Store result to EDX
    
    FLD EAX          ; Load 100.0 again
    FLD EBX          ; Load 10.0 again
    FMUL            ; ST0 = ST0 * ST1 = 10.0 * 100.0 = 1000.0
    FST ESI          ; Store result to ESI
    
    FLD EAX          ; Load 100.0 again
    FLD EBX          ; Load 10.0 again
    FDIV            ; ST0 = ST0 / ST1 = 10.0 / 100.0 = 0.1
    FST EDI          ; Store result to EDI
    
    ; Test 4: Mathematical Functions
    LOAD_IMM64 ESP, 0x4040000000000000  ; Load 32.0
    FLD ESP          ; Load 32.0 to FPU stack
    FSQRT           ; ST0 = sqrt(32.0) ≈ 5.657
    FST EBP          ; Store result to EBP
    ...
    OUT ECX          ; Should show 110.0
    OUT EDX          ; Should show -90.0  
    OUT ESI          ; Should show 1000.0
    OUT EDI          ; Should show 0.1
    OUT EBP          ; Should show ~5.657
